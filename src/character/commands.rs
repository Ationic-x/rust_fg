    ...
    for (index, element) in elements.iter().enumerate() {
        ...
        if let Some(command) = commands.get_mut(pos) {
            ...
            if let Some(cmd_element) =
                Some(CommandElement::new(inputs, time_value, sensitive, &directions_and_actions))
            {
...
pub fn create_command_tree(char_name: &str) -> CommandNode {
    let assets = std::env::current_dir().unwrap().join("src").join("assets");
    let mut tree = CommandNode::new();

    match read_command_file(assets.join(char_name).to_str()) {
        Ok(commands) => {
